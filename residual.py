...
class SKSAutoencoderResidual(nn.Module):
    def __init__(self, input_dim, hidden_dim, k, n_of_transitional_layers=3):
        super(SKSAutoencoderResidual, self).__init__()
        ...
        # Aggiungiamo il livello Dropout dopo la creazione dell'encoder
        self.dropout = nn.Dropout(p=0.5)  # 50% dropout rate

        self.attention = SelfAttention(hidden_dim)
        self.k_sparse = KSparse(k)

        # Proiezione lineare per adattare la dimensione del residuo a quella di z
    ...
    def forward(self, inputs):
        ...
        residual = residual.view(residual.size()[0], -1)  # Flatten del tensor (necessario per i layer fully connected)
        residual = self.residual_projection(residual)  # Proietta il residuo per farlo combaciare con z

        z += residual  # Residual connection
        reconstructed = self.decode(z)  # Decoder
        return reconstructed

    def encode(self, x):
        self.__set_decoder_dimension(x.size())  # Necessario per il decoder
        x = x.view(x.size()[0], -1)  # Flatten del tensor (necessario per i layer fully connected)

        x = self.encoder(x)
        x = self.dropout(x)  # Applichiamo il dropout dopo l'encoder
        x = self.attention(x)
        x = self.k_sparse(x)
        return x
